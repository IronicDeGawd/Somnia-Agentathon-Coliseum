 ...
 *
 *   TURNS=3 FIGHTER=1 pnpm exec hardhat run scripts/queue-opponent.ts --network somnia
 *
...
async function main() {
  ...
  const simulated = process.env.SIMULATED === "1";
  ...
  const required = (await mm.read.halfDeposit([turns, simulated])) as bigint;
  ...
  const slotBefore = (await mm.read.getSlot([turns, simulated])) as unknown[];
  console.log(`Waiting player: ${slotBefore[0]} (fighter ${slotBefore[1]})`);

  // queue takes the fighter FIRST, then the tier.
  const tx = await mm.write.queue([fighter, turns, simulated]);
