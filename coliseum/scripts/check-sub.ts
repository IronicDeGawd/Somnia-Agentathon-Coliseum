...
async function main() {
  ...
  for (const subId of [2133461n, 2135845n]) {
    try {
      ...
      console.log(`sub ${subId}:`, JSON.stringify(res, (_, v) => typeof v === "bigint" ? v.toString() : v, 2));
    } catch (e: any) {
      console.log(`sub ${subId}: ERROR — ${e.shortMessage || e.message}`);
    }
  }
}
